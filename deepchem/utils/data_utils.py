...
def load_sdf_files(
    ...
    for input_file in input_files:
        ...
        suppl = Chem.SDMolSupplier(
            str(input_file), sanitize=clean_mols, removeHs=False, strictParsing=False
        )
        for ind, mol in enumerate(suppl):
            ...
            if shard_size is not None and len(df_rows) == shard_size:
                if has_csv:
                    mol_df = pd.DataFrame(
                        df_rows,
                        columns=("mol_id", "smiles", "mol", "pos_x", "pos_y", "pos_z"),
                    )
                    raw_df = next(
...
def load_csv_files(
    input_files: List[str], shard_size: Optional[int] = None
) -> Iterator[pd.DataFrame]:
...
def load_dataset_from_disk(
    ...
    bool,
    Optional[
        Tuple["dc.data.DiskDataset", "dc.data.DiskDataset", "dc.data.DiskDataset"]
    ],
...
def save_transformers(save_dir: str, transformers: List["dc.trans.Transformer"]):
...
def remove_missing_entries(dataset):
    ...
    for i, (X, y, w, ids) in enumerate(dataset.itershards()):
        ...
        logger.info(
            "Shard %d has %d missing entries." % (i, np.count_nonzero(~available_rows))
        )
        X = X[available_rows]
        y = y[available_rows]
        w = w[available_rows]
