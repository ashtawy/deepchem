...
def receptor_to_pdbqt(receptor_file_name, receptor_pdbqt_file_name, overwrite=False):
    ...
    if not os.path.exists(receptor_pdbqt_file_name) or overwrite:
        try:
            ...
            python_path = "/home/hashtawy/.conda/envs/autodock_tools/bin/python"
            command = f"{python_path} {conv_path} -U waters -r {receptor_file_name} -o {receptor_pdbqt_file_name}"
            output = subprocess.run(command,
                                env = env_vars, 
                                stdout=subprocess.PIPE, 
                                stderr=subprocess.PIPE,
            ...
            print(f"Failed to convert receptor to pdbqt using prepare_receptor4.py: {receptor_file_name}")
            #print(f"Could not convert receptor from pdb to the PDBQT format using prepare_receptor4.py: {receptor_file_name}")
            #try:
            #    print("tyring with openbabel")
            #    #subprocess.call(["obabel", receptor_file_name, f"-O{receptor_pdbqt_file_name}"])
            #except:
            #    print(f"Could not convert receptor from pdb to the PDBQT format using obabel: {receptor_file_name}")
...
class PDB:
    ...
    def LoadPDB_from_file(self, FileName, line_header="", verbose=False):
        ...
        file.close()
        self.LoadPDB_from_list(lines, self.line_header, verbose)

    def LoadPDB_from_list(self, lines, line_header="", verbose=False):
    ...
    def CheckProteinFormat_process_residue(self, residue, last_key):
        ...
        if self.verbose and real_resname in self.protein_resnames: # so it's a protein residue

            if not "N" in residue and self.verbose:
                ...
                print("")
            if not "C" in residue and self.verbose:
                ...
                print("")
            if not "CA" in residue and self.verbose:
            ...
            if real_resname == "GLU" or real_resname == "GLH" or real_resname == "GLX":
                if not "OE1" in residue and self.verbose:
                    ...
                    print("")
                if not "OE2" in residue and self.verbose:
            ...
            if real_resname == "ASP" or real_resname == "ASH" or real_resname == "ASX":
                if not "OD1" in residue and self.verbose:
                    ...
                    print("")
                if not "OD2" in residue and self.verbose:
            ...
            if real_resname == "LYS" or real_resname == "LYN":
                if not "NZ" in residue and self.verbose:
            ...
            if real_resname == "ARG":
                if not "NH1" in residue and self.verbose:
                    ...
                    print("")
                if not "NH2" in residue and self.verbose:
            ...
            if real_resname == "HIS" or real_resname == "HID" or real_resname == "HIE" or real_resname == "HIP":
                if not "NE2" in residue and self.verbose:
                    ...
                    print("")
                if not "ND1" in residue and self.verbose:
            ...
            if real_resname == "PHE":
                if not "CG" in residue and self.verbose:
                    ...
                    print("")
                if not "CD1" in residue and self.verbose:
                    ...
                    print("")
                if not "CD2" in residue and self.verbose:
                    ...
                    print("")
                if not "CE1" in residue and self.verbose:
                    ...
                    print("")
                if not "CE2" in residue and self.verbose:
                    ...
                    print("")
                if not "CZ" in residue and self.verbose:
            ...
            if real_resname == "TRP":
                ...
                if not "CD2" in residue and self.verbose:
                    self.printout('WARNING: There is no atom named "CD2" in the protein residue ' + last_key + '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "NE1" in residue and self.verbose:
                ...
                if not "CE2" in residue and self.verbose:
                    self.printout('WARNING: There is no atom named "CE2" in the protein residue ' + last_key + '. Please use standard naming conventions for all protein residues. This atom is needed to determine pi-pi and pi-cation interactions. If this residue is far from the active site, this warning may not affect the NNScore.')
                    print("")
                if not "CE3" in residue and self.verbose:
                    ...
                    print("")
                if not "CZ2" in residue and self.verbose:
                    ...
                    print("")
                if not "CZ3" in residue and self.verbose:
                    ...
                    print("")
                if not "CH2" in residue and self.verbose:
...
class binana:
    ...
    def append_features(self, global_features, new_features, feature_set_name):
        for feature_name, feature_value in new_features.items():
            global_features[feature_set_name + "__" + feature_name] = feature_value
    ...
    def featurize(self, receptor, ligand, line_header, verbose=False): # must be a more elegant way of doing this
        ...
        if isinstance(receptor, PDB):
            ...
            base_name, extension = os.path.splitext(receptor)
            if extension == ".pdbqt":
                receptor_pdbqt_filename = receptor
            else:
                receptor_pdbqt_filename = receptor.replace(extension, ".pdbqt")
                receptor_to_pdbqt(receptor, receptor_pdbqt_filename)
            if os.path.exists(receptor_pdbqt_filename) is False:
                raise ValueError("The receptor PDBQT file " + receptor_pdbqt_filename + " does not exist.")
            receptor = PDB(self.verbose)
            receptor.LoadPDB_from_file(receptor_pdbqt_filename, verbose=self.verbose)
            receptor.OrigFileName = receptor_pdbqt_filename


        receptor.assign_secondary_structure()

        if isinstance(ligand, PDB):
        ...
        self.features = {}
        self.features = self.append_features(self.features, self.ligand_receptor_atom_type_pairs_less_than_four, 
                                        "ligand_receptor_atom_type_pairs_less_than_four")
        self.features = self.append_features(self.features, self.ligand_receptor_atom_type_pairs_electrostatic,
                                        "ligand_receptor_atom_type_pairs_electrostatic")
        self.features = self.append_features(self.features, self.ligand_atom_types, "ligand_atom_types")
        self.features = self.append_features(self.features, self.ligand_receptor_atom_type_pairs_less_than_two_half,
                                        "ligand_receptor_atom_type_pairs_less_than_two_half")
        self.features = self.append_features(self.features, self.hbonds, "hbonds")
        self.features = self.append_features(self.features, self.hydrophobics, "hydrophobics")
        self.features = self.append_features(self.features, self.stacking, "stacking")
        self.features = self.append_features(self.features, self.pi_cation, "pi_cation")
        self.features = self.append_features(self.features, self.t_shaped, "t_shaped")
        self.features = self.append_features(self.features, self.active_site_flexibility, "active_site_flexibility")
        self.features = self.append_features(self.features, self.salt_bridges, "salt_bridges")
        self.features = self.append_features(self.features, self.rotateable_bonds_count, "rotateable_bonds_count")
