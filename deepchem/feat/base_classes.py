        ...
        for i, mol in enumerate(datapoints):
            ...
            try:
                if isinstance(mol, str):
                    ...
                    if (
                        hasattr(self, "use_original_atoms_order")
    ...
    def featurize(
        ...
        if n_threads > 1 and len(datapoints) >= n_threads:
            ...
            for i, from_i in enumerate(range(0, max_steps, step_size)):
                ...
                smiles_chunks = np.array_split(datapoints_i, n_threads)
                smiles_idxs = np.array_split(np.arange(len(datapoints_i)), n_threads)

                kwargs_list = [kwargs] * n_threads
                log_e_n_list = [log_every_n] * n_threads
                args = list(zip(smiles_idxs, smiles_chunks, log_e_n_list, kwargs_list))
...
class MaterialStructureFeaturizer(Featurizer):
    ...
    def featurize(
        self,
        datapoints: Optional[Iterable[Union[Dict[str, Any], PymatgenStructure]]] = None,
