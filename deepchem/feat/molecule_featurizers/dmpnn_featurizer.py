...
class GraphConvConstants(object):
    ...
    FEATURE_GENERATORS: Dict[str, MolecularFeaturizer] = {
        ...
        "morgan_count": CircularFingerprint(
            radius=2, size=2048, sparse=False, is_counts_based=True
        ),
        "rdkit_desc": RDKitDescriptors(use_bcut2d=False),
        "rdkit_desc_normalized": RDKitDescriptors(use_bcut2d=False, is_normalized=True),
    }
...
def map_reac_to_prod(
    mol_reac: RDKitMol, mol_prod: RDKitMol
...
class _MapperDMPNN:
    ...
    def __init__(
        self, datapoint: RDKitMol, concat_fdim: int, f_atoms_zero_padded: np.ndarray
    ):
        """
        Parameters
        ----------
    ...
    def _generate_mapping(self):
        ...
        self.mapping = np.asarray(self.atom_to_incoming_bonds)[self.bond_to_ini_atom]
    ...
    def _extend_concat_feature(self, a1: int, bond_feature: np.ndarray):
        ...
        concat_input: np.ndarray = np.concatenate(
            (self.f_atoms_zero_padded[a1], bond_feature), axis=0
        ).reshape([1, self.concat_fdim])
    ...
    def _modify_based_on_max_bonds(self):
        ...
        max_num_bonds: int = max(
            1,
            max(len(incoming_bonds) for incoming_bonds in self.atom_to_incoming_bonds),
        )
        self.atom_to_incoming_bonds = [
            self.atom_to_incoming_bonds[a]
            + [0] * (max_num_bonds - len(self.atom_to_incoming_bonds[a]))
...
def generate_global_features(
    mol: RDKitMol, features_generators: List[str]
) -> np.ndarray:
    """
    ...
    replace_token = 0
    global_features_array = np.where(
        np.isnan(global_features_array), replace_token, global_features_array
    )
...
class DMPNNFeaturizer(MolecularFeaturizer):
    ...
    def _featurize(self, datapoint: RDKitMol, **kwargs) -> GraphData:
        ...
        if self.master_atom:
            f_atoms, f_bonds, edge_index = self.add_master_atom(
                f_atoms, f_bonds, edge_index
            )
        ...
        if self.features_generators is not None:
            global_features = generate_global_features(
                datapoint, self.features_generators
            )
