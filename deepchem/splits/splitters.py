    ...
    def k_fold_split(
        ...
        **kwargs,
        ...
        if isinstance(dataset, DiskDataset):
            ...
            rem_dataset = DiskDataset.from_numpy(
        ...
        for i in range(max_count):
            for task in range(n_tasks):
                ...
                if i < len(indices) and indices[i] not in assigned:
                    ...
                    set_frac = [
                        1
                        if set_target[i][task] == 0
                        else set_counts[i][task] / set_target[i][task]
                        for i in range(3)
                    ]
...
class SingletaskStratifiedSplitter(Splitter):
    ...
    def split(
        ...
        np.testing.assert_equal(frac_train + frac_valid + frac_test, 1.0)
        np.testing.assert_equal(
            10 * frac_train + 10 * frac_valid + 10 * frac_test, 10.0
        )
...
class IndexSplitter(Splitter):
    ...
    def split(
        ...
        return (
            indices[:train_cutoff],
            indices[train_cutoff:valid_cutoff],
...
class MolecularWeightSplitter(Splitter):
    ...
    def split(
        ...
        return (
            sortidx[:train_cutoff],
            sortidx[train_cutoff:valid_cutoff],
...
class MaxMinSplitter(Splitter):
    ...
    def split(
        ...
        testIndices = picker.LazyPick(
            distFunc=distance, poolSize=num_datapoints, pickSize=num_test, seed=seed
        )
...
class ButinaSplitter(Splitter):
    ...
    def split(
        ...
        scaffold_sets = Butina.ClusterData(dists, nfps, self.cutoff, isDistData=True)
...
def _generate_scaffold(smiles: str, include_chirality: bool = False) -> str:
    ...
    try:
        from rdkit import Chem
        from rdkit.Chem.Scaffolds import MurckoScaffold as BMS
    except ModuleNotFoundError:
        raise ImportError("This function requires RDKit to be installed.")

    mol = Chem.MolFromSmiles(smiles)
    mol_scaffold = BMS.GetScaffoldForMol(mol)
    mol_generic_scaffold = BMS.MakeScaffoldGeneric(mol_scaffold)
    smiles_generic_scaffold = Chem.MolToSmiles(mol_generic_scaffold)

    return smiles_generic_scaffold
...
class FingerprintSplitter(Splitter):
    ...
    def split(
        ...
        train_inds, test_valid_inds = _split_fingerprints(
            fps, train_size, valid_size + test_size, 1
        )
        ...
        if valid_size == 0 or frac_valid == 0:
            ...
            test_inds, valid_inds = _split_fingerprints(
                test_valid_fps, test_size, valid_size, 1
            )
...
class SimilaritySplitter(Splitter):
    ...
    def __init__(self, cutoff=0.4):
        """Create a FingerprintSplitter."""
        super(SimilaritySplitter, self).__init__()
        self.cutoff = cutoff
    ...
    def split(
        ...
        test_size = len(dataset) - train_size - valid_size
        train_inds, test_valid_inds = _split_fingerprints(
            fps, train_size, valid_size + test_size, self.cutoff
        )
        ...
        if valid_size == 0:
            ...
            test_valid_fps = [fps[i] for i in test_valid_inds]
            test_inds, valid_inds = _split_fingerprints(
                test_valid_fps, test_size, valid_size, self.cutoff
            )
            test_inds = [test_valid_inds[i] for i in test_inds]
            valid_inds = [test_valid_inds[i] for i in valid_inds]
        return train_inds, valid_inds, test_inds
...
def _split_fingerprints(
    fps: List, size1: int, size2: int, cutoff: float
    ...
    lowest_sim = -1
    while len(remaining_fp) > 0 and lowest_sim < cutoff:
        ...
        lowest_sim = np.min(max_similarity_to_group[1 - group])
...
class ScaffoldSplitter(Splitter):
    ...
    def generate_scaffolds(
        ...
        max_scaffold_freq = -1
        most_freq_scaffold = None
        for ind, smiles in enumerate(dataset.ids):
            if ind % log_every_n == 0:
                logger.info(f"Generating scaffold {ind}/{data_len}")
            scaffold = _generate_scaffold(smiles)
            if scaffold not in scaffolds:
                scaffolds[scaffold] = [ind]
            else:
                scaffolds[scaffold].append(ind)
            if len(scaffolds[scaffold]) > max_scaffold_freq:
                max_scaffold_freq = len(scaffolds[scaffold])
                most_freq_scaffold = scaffold
        logger.info(
            f"Number of unique scaffols = {len(scaffolds)} in a dataset of {len(dataset.ids)} compounds"
        )
        logger.info(
            f"Most frequent scaffold `{most_freq_scaffold}` appeared in {max_scaffold_freq} compounds"
        )
