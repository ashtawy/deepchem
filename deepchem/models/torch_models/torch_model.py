...
from deepchem.models.optimizers import Adam, LearningRateSchedule, Optimizer
...
class TorchModel(Model):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if self.tensorboard:
            self._summary_writer = torch.utils.tensorboard.SummaryWriter(self.model_dir)
    ...
    def predict_on_batch(
        self, X: np.typing.ArrayLike, transformers: List[Transformer] = []
    ) -> OneOrMany[np.ndarray]:
    ...
    def predict(
        ...
        if it produces multiple outputs
        """
        generator = self.default_generator(dataset, mode="predict", pad_batches=False)
    ...
    def predict_uncertainty(
        self, dataset: Dataset, masks: int = 50
    ) -> OneOrMany[Tuple[np.ndarray, np.ndarray]]:
        """
        Predict the model's outputs, along with the uncertainty in each one.
    ...
    def compute_saliency(self, X: np.ndarray) -> OneOrMany[np.ndarray]:
        ...
        for output in outputs:
            ...
            for i in range(output.shape[0]):
                ...
                X_tensor.grad.zero_()
            final_result.append(
                torch.reshape(torch.stack(result), output_shape + input_shape)
                .cpu()
                .numpy()
            )
    ...
    def _create_assignment_map(
        self, source_model: "TorchModel", include_top: bool = True, **kwargs
    ) -> Dict[Any, Any]:
        """
    ...
    def _create_value_map(self, source_model: "TorchModel", **kwargs) -> Dict[Any, Any]:
        """
